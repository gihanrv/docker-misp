#!/bin/bash
    ...
    if [ ! -f /var/lib/mysql/.db_initialized ]; then
            ...
            mysql -h$KMYSQL_HOST -uroot -p$KMYSQL_ROOT_PASSWORD -e "CREATE USER $KMYSQL_USER@@'%' IDENTIFIED BY '$KMYSQL_DATABASE'"
            ...
            mysql -h$KMYSQL_HOST -u$KMYSQL_USER -p$KMYSQL_PASSWORD  < /var/www/MISP/INSTALL/MYSQL.sql
